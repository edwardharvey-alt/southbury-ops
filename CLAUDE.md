...
- T-fundraising-order-count-single-source — `order_count` is derived independently by both `v_drop_summary` and `v_drop_fundraising_summary`, from character-identical predicates that agree today (verified 2026-07-19). Not a live bug; drift prevention. `fundraising_total_pence` is computed from the fundraising view's count but displayed beside the summary view's, so changing one exclusion list and not the other would make a host page show an order count and a fundraising total that silently disagree. Fix: have `v_drop_summary` take `COALESCE(fs.order_count, 0)` like it already does for `drop_gmv_pence`. Post-launch, low priority. — open
- T-fundraising-composed-line-consumers — `fundraising_display_text` is now an OPTIONAL override of a line composed from `fundraising_model` + amount + `fundraising_cause_name`. Drop Studio composes and previews it client-side, but `host-view.html` / `host-view-summary` still render the raw display text, so a drop saved with a blank override renders nothing there. Build the line server-side as a `fundraising_descriptor` (mirroring the existing `host_share_descriptor` pattern), re-point host-view, and add the customer-facing line to `order.html`. Never project `fundraising_cause_reference` — operator-only. Do before the first real fundraising drop with a blank message. — open
