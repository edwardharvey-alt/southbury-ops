...
- T-brand-page-field-grouping — group the Brand page by what each field does. Business name/Tagline (published to the ordering page, revisable), the QR card line (printed, fixed until reprint) and "In your own words" (never shown to customers — a voice sample Hearth generates from) all render as identical text fields. The third distinction is unstated and the current hint reads as though it IS published. Fix = three headed sections with one-line descriptions; "Never shown to customers. Hearth reads this to sound like you." is the load-bearing one. Surfaced by T-CAP-2b PR1; kept out of PR2. Post-launch, small. — open
...
- T-fundraising-order-count-single-source — `order_count` is derived independently by both `v_drop_summary` and `v_drop_fundraising_summary`, from character-identical predicates that agree today (verified 2026-07-19). Not a live bug; drift prevention. `fundraising_total_pence` is computed from the fundraising view's count but displayed beside the summary view's, so changing one exclusion list and not the other would make a host page show an order count and a fundraising total that silently disagree. Fix: have `v_drop_summary` take `COALESCE(fs.order_count, 0)` like it already does for `drop_gmv_pence`. Parked pending a test fixture carrying both a `pending_payment` and a `cancelled` order — the two predicates only diverge on the rows the exclusion lists touch, so verifying the fix needs a drop that actually has them. Post-launch, low priority. — open
- T-fundraising-fixed-contribution-guard — a FIXED fundraising contribution (`per_order`, `per_item`) can exceed what the order is worth; `percentage` is self-limiting and out of scope. **Live-verification finding: the real ceiling is the vendor's NET proceeds, not the order total** — Stripe's processing fee (~1.4% + 20p) and Hearth's platform `application_fee_amount` (1.5% + 20p, PR #474) both come out first. Worked case: a £1 order at `per_item` £1/item = £1 fundraising + ~22p platform + ~21p Stripe ≈ **£1.43 of obligations on a £1 charge**, and the fixed 20p components make the gap proportionally worst at exactly the small order sizes where a flat pledge already bites hardest. A guard clamping at the order total would still leave the vendor underwater — when built it must reckon with fees. Today there is only the non-blocking per-model Drop Studio advisory (`renderFundraisingAmountWarning()`), which compares against the cheapest enabled item and goes silent when no items are enabled; no server-side guard exists (fundraising never touches the money path — it is reporting computed in views after the fact, so the exposure is to vendor margin only). NOT a go-live blocker — real vendors at real order sizes never approach it. Post-launch, low priority. — open
...
- 2026-07-20: Fundraising `per_item` model complete (#488 data layer + PR 2
  ...
  **Verified end-to-end on live before closure:** all three models compute
  correctly across Drop Studio, the order page, checkout, the confirmation page
  and email, the host view, and the vendor Scorecard / Home — penny-verified
  against `SUM(qty) × amount` on a real order, with the private
  `fundraising_cause_reference` confirmed absent from every customer- and
  host-facing payload.
  Verification also produced one new finding, logged as
  **T-fundraising-fixed-contribution-guard**: the ceiling on a FIXED contribution
  is the vendor's NET proceeds, not the order total, because Stripe's processing
  fee and Hearth's platform `application_fee_amount` come out first (a £1 order at
  £1/item carries ≈ £1.43 of obligations). Not a go-live blocker; it binds
  whenever the MOV / cap guard is built.
  **The fundraising cluster is now closed but for two post-launch items:**
  T-fundraising-order-count-single-source (drift-prevention only — the two
  derivations agree today; parked pending a `pending_payment` + `cancelled` test
  fixture to exercise the exclusion lists) and T-fundraising-notes-overlap
  (dormant-column retirement decision), plus the new guard ticket above.
