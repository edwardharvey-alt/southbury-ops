...
- T-CAP-2b PR1 — vendor QR card line: ✓ SHIPPED 2026-07-21. Nullable `vendors.qr_card_line` (migration `20260721130000`) + `update-vendor` whitelist/validation + Brand page field below Tagline (Cormorant italic, maxlength 60, live counter, three static examples). Data layer only — the QR generator and rendered card are PR2. NULL is a supported end state (card reads fine without it); no AI generation, by decision. Over-length rejected, never truncated; whitespace-only coerces to NULL.
