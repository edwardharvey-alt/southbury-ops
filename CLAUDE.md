...
- T-ops-rls-fix — Service Board order status transitions silently fail in production (Healthy Habits blocker, equal priority to T3-8). The `orders` RLS policies "Orders: authenticated owner select" and "Orders: authenticated owner update" require `auth.uid()` to match `vendors.auth_user_id`, but the Service Board runs anon — every status UPDATE returns 204 and affects 0 rows. All four transitions (Confirm, Bake, Ready, Delivered) are affected. Fix path: build a `transition-order-status` Edge Function mirroring the `transition-drop-status` pattern (service-role bypass with in-function authorisation via `supabase.auth.getUser()` and vendor-ownership check). Faster path than the full T5-A auth workstream. Verified 2026-05-14 with order `8f56908e-3c3c-4407-b306-2a235c63d4db`.
